  ...
  useEffect(() => {
    ...
    if (user && token && hayConexion) {
      ...
      newSocket.on('new_message', async (serverMsg) => {
        try {
          // Mapeamos explícitamente para evitar errores de nombres
          await MessageQueries.saveMessage({ ...serverMsg, is_synced: 1, status: 'received' });
          ...
          //emitimos un evento para refrescar los mensajes
      ...
      newSocket.on('message_status_changed', async (serverMsg) => {
        try {
          ...
          console.log("solicitud de cambio de estado de mensaje entrante en cliente "+user.id+" -> " + JSON.stringify(serverMsg));
          //actualizamos el estado del mensaje en la base de datos local
          await MessageQueries.updateMessageStatus({ messageId: serverMsg.messageId, status: serverMsg.status });
