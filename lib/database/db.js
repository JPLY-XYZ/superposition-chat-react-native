...
const db = SQLite.openDatabaseSync('mylocal31.db');
...
ORDER BY c.updatedAt DESC`,
  ...
  setLastConversationMessage: async (conversationId, BasicMessage) => {
    await acquireLock();
    try {
      const now = new Date().toISOString();


      // Suponemos que si tú ejecutas esto, el senderId es el 'myId' 
      // que puedes obtener de tu estado global o AuthContext antes de llamar a la función.
      ...
      await db.runAsync(
       ...
       SET 
         ...
         updatedAt = ?
       WHERE id = ?`,
        [JSON.stringify(BasicMessage), now, conversationId]
      );
